...
const Sidebar = ({ isCollapsed, setIsCollapsed }: SidebarProps) => {
  ...
  const linkClass = ({ isActive }: { isActive: boolean }) =>
    `flex items-center gap-3 px-3 py-2.5 rounded-xl transition-all duration-200 group ${isActive
      ? "bg-primary/10 text-primary font-bold shadow-sm" // Active: Primary light bg, primary text
  ...
  return (
    ...
    >
      ...
      <div className="mt-auto pt-4 border-t border-gray-100">
        <button
          onClick={signout}
          className={`flex items-center gap-3 px-3 py-3 rounded-xl font-medium text-red-500 hover:bg-red-50 hover:text-red-600 w-full transition-all group ${isCollapsed ? "justify-center" : ""
            }`}
          title="Logout"
        >
