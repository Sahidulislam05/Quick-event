...
const UserDashboard = () => {
  ...
  const totalSpent = confirmedBookings.reduce(
    (sum, item) => sum + parseInt(item.price as unknown as string),
    0
  );
      ...
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          ...
          <div className="group bg-white p-8 rounded-2xl shadow-sm hover:shadow-md transition-all duration-300 border border-gray-100">
            <div className="flex items-center justify-between mb-6">
              <div
                className={`p-3 rounded-xl transition-colors duration-300 ${user?.emailVerified
                ...
                <tbody className="text-sm">
                  {bookings.slice(0, 5).map((item) => (
                    ...
                    >
                      ...
                      <td className="px-8 py-4">
                        <span
                          className={`inline-flex items-center gap-2 px-3 py-1 rounded-full text-xs font-semibold capitalize ${item.status === "confirmed"
                            ? "bg-lime-100 text-lime-700"
                            : "bg-orange-100 text-orange-700"
                            }`}
                        >
                          <span
                            className={`w-1.5 h-1.5 rounded-full ${item.status === "confirmed"
                              ? "bg-lime-500"
                              : "bg-orange-500"
                              }`}
                          ></span>
